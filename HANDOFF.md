...
- **i18n**: i18next + react-i18next + i18next-browser-languagedetector（中/英双语）
...
23. **WhatsApp 深链接** — 电话号码旁绿色 WhatsApp 按钮，点击跳转 `wa.me/{phone}`，CardDetail 只读模式 + RecognizePreview 预览弹窗均支持

## i18n 架构（src/lib/i18n.ts）

```
翻译文件：src/locales/zh.json + en.json（~180 key）
Key 命名：扁平 namespace，如 app.xxx、auth.xxx、field.xxx、placeholder.xxx
检测顺序：localStorage('i18nextLng') → navigator.language
  - zh 开头 → 中文
  - 其他 → 英文（fallbackLng）
切换方式：Header 右上角按钮，调用 i18n.changeLanguage()，自动存 localStorage
组件用法：const { t } = useTranslation(); t('key')
动态插值：t('export.selected', { selected: 3, total: 10 })
```
...
- WhatsApp `wa.me/{phone}` 深链接：无需安装、无需添加联系人即可发起聊天
...
- **iOS Safari `window.open(vcf blob)`** — 会弹系统确认弹窗 "is trying to download a contact card"，体验比 `<a download>` 更差，已还原
- **Web 直接写入 iOS 通讯录** — 苹果不允许 Web 访问 CNContactStore，只有原生 App 才能静默保存到通讯录
...
| `src/App.tsx` | 主状态管理 + 视图路由 + AuthProvider + Tab Bar + 语言切换 + 登录后补存逻辑 |
...
| `src/lib/vcard.ts` | vCard 3.0 生成 + 文件下载 + `getWhatsAppUrl()` |
...
| `src/components/CardDetail.tsx` | 名片详情页（查看 + 编辑模式 + WhatsApp 按钮 + 缓存优先图片） |
...
| `src/components/RecognizePreview.tsx` | 识别完成预览弹窗（含 WhatsApp 按钮） |
| `src/components/CardEditor.tsx` | 编辑视图（左图右表单） |
| `src/components/UploadZone.tsx` | 上传区（拖拽 + 相机 + 相册） |
| `src/components/ContactForm.tsx` | 编辑表单（字段组 + 动态邮箱/电话列表） |
...
- 原生 App 包壳（Capacitor）— 如需 iOS 静默保存通讯录功能，需包成原生 App 调用 CNContactStore
